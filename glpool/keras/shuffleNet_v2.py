...
def interpolation_upsample(input_batch):
    '''
    Realize upsample via Interpolation operation.
    '''
    height = input_batch.get_shape().as_list()[1]
    new_size = height-2
    new_batch = tf.image.resize_images(input_batch, (new_size, new_size), method = ResizeMethod.BILINEAR)
    return new_batch

def interpolation_upsample_2(input_batch):
    '''
    Realize upsample via Interpolation operation.
    '''
    height = input_batch.get_shape().as_list()[1]
...
def pglPool(inputs, name):
    h1 = inputs.get_shape().as_list()[1]
    x1 = bn_dwconv_valid(inputs, h1, 1, name = '{}_glp1'.format(name))

    x2 = layers.Lambda(interpolation_upsample, name = '{}_glp20'.format(name))(inputs)
    h2 = x2.get_shape().as_list()[1]
    x2 = bn_dwconv_valid(x2, h2, 1, name = '{}_glp21'.format(name))

    x3 = layers.Lambda(interpolation_upsample_2, name = '{}_glp30'.format(name))(inputs)
    h3 = x3.get_shape().as_list()[1]
    x3 = bn_dwconv_valid(x3, h3, 1, name = '{}_glp31'.format(name))

    out = layers.Concatenate(-1, name = '{}_glp_cat'.format(name))([x1,x2,x3])
    return out
...
def ShuffleNet_V2():
    ...
    #x = bn_dwconv_valid(x, 8, 1, name = 'GDW')
    x = glpool_layer.GLPooling2D(pool_size = (8,8))(x)
    # # 1 * 1 * 1024
